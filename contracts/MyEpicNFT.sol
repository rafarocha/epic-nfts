...
contract MyEpicNFT is ERC721URIStorage {
  ...
  uint256 public maxNFTs = 50;
  uint256 public totalNFTsMintedSoFar = 0;
  ...
  string[] firstWords = ["Seiya", "Shiryu", "Hyoga", "Ikki", "Athena", "Saori"];
  string[] secondWords = ["Shun", "Jabu", "Ichi", "Nachi", "Geki", "Ban"];
  string[] thirdWords = ["Mime", "Docrates", "Aiolia", "Shaka", "Milo", "Camus"];

  event NewEpicNFTMinted(address sender, uint256 tokenId);

  constructor() ERC721 ("ChavesNFT", "CHAVO") {
    require(totalNFTsMintedSoFar < maxNFTs, "Numero maximo de NFTs ja foi cunhado");

    console.log("Meu contrato de NFT! Tchu-hu");
    totalNFTsMintedSoFar++;
  }
  ...
  function criarNFT(string memory _name, string memory _description, string memory _imageURI) public {
    uint256 newNFTId = nfts.length;
    nfts.push(NFT(newNFTId, _name, _description, _imageURI));
  }
